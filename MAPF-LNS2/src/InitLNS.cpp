...
bool InitLNS::run() {
  ...
  while (runtime < time_limit and num_of_colliding_pairs > 0) {
    assert(
        instance.validateSolution(paths, sum_of_costs, num_of_colliding_pairs));
      ...
      if (neighbor.colliding_pairs.size() < neighbor.old_colliding_pairs.size())
        destroy_weights[selected_neighbor] =
            reaction_factor * (double)(neighbor.old_colliding_pairs.size() -
                                       neighbor.colliding_pairs
                                           .size())  // / neighbor.agents.size()
    ...
    if (screen >= 2)
      cout << "New colliding pairs = " << neighbor.colliding_pairs.size()
           << endl;
    ...
    if (screen >= 1)
      cout << "Iteration " << iteration_stats.size() << ", "
           ...
           << "remaining time = " << time_limit - runtime << endl;
    iteration_stats.emplace_back(neighbor.agents.size(), sum_of_costs, runtime,
                                 replan_algo_name, 0, num_of_colliding_pairs);
  }
  ...
  while (p != shuffled_agents.end() &&
    ...
    if (screen >= 3) {
      ...
      cout << "After agent " << id
           ...
           << ", remaining time = " << time_limit - runtime << " seconds. "
           << endl;
    }
    if (neighbor.colliding_pairs.size() >= neighbor.old_colliding_pairs.size())
      break;
...
bool InitLNS::updateCollidingPairs(set<pair<int, int>>& colliding_pairs,
  ...
  for (int t = 1; t < (int)path.size(); t++) {
    ...
    {  // this agent traverses the target of another agent
      for (auto id :
           path_table.table[to][path_table.goals[to]])  // look at all agents at
                                                        // the goal time
      {
...
bool InitLNS::generateNeighborByCollisionGraph() {
  ...
  if ((int)G.size() < neighbor_size)
  {
      ...
      {
          int a1 = *std::next(neighbors_set.begin(), rand() %
  neighbors_set.size()); int a2 = randomWalk(a1); if (a2 != NO_AGENT)
  ...
  if (screen >= 2)
      cout << "Generate " << neighbor.agents.size() << " neighbors by collision
  graph" << endl; return true;*/
...
void InitLNS::writeResultToFile(const string& file_name, int sum_of_distances,
  ...
  stats << runtime << "," << iteration_stats.back().num_of_colliding_pairs
        ...
        << num_LL_expanded << "," << num_LL_generated << "," << num_LL_reopened
        << "," << num_LL_runs << "," << preprocessing_time << ","
