...
class ConstraintTable {
 public:
  ...
  int getHoldingTime(int location, int earliest_timestep)
      const;  // the earliest timestep that the agent can hold the location
              // after earliest_timestep
  ...
  ConstraintTable(size_t num_col, size_t map_size,
                  const PathTable* path_table_for_CT = nullptr,
  ...
  void insert2CT(const HLNode& node,
                 int agent);  // build the constraint table for the given agent
                              // at the give node
  void insert2CT(const list<Constraint>& constraints,
                 int agent);  // insert constraints for the given agent to the
                              // constraint table
  void insert2CT(const Path& path);  // insert a path to the constraint table
  void insert2CT(
      size_t loc, int t_min,
      int t_max);  // insert a vertex constraint to the constraint table
  void insert2CT(
      size_t from, size_t to, int t_min,
      int t_max);  // insert an edge constraint to the constraint table
  void insert2CAT(int agent,
                  const vector<Path*>& paths);  // build the conflict avoidance
                                                // table using a set of paths
  void insert2CAT(
      const Path&
          path);  // insert a path to the collision avoidance table
 ...
 protected:
  ...
  map<int, size_t> landmarks;  // <timestep, location>: the agent must be at the
