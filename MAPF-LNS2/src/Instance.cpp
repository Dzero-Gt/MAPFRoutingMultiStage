...
Instance::Instance(const string& map_fname, const string& agent_fname, 
	int num_of_agents, int num_of_rows, int num_of_cols, int num_of_obstacles, int warehouse_width, int minSeparation):
	map_fname(map_fname), agent_fname(agent_fname), num_of_agents(num_of_agents), minAgentSeparation(minSeparation)
{
	bool succ = loadMap();
	if (!succ)
	{
...
bool Instance::validateSolution(const vector<Path*>& paths, int sum_of_costs, int num_of_colliding_pairs) const
{
    ...
    for (auto i = 0; i < start_locations.size(); i++)
    {
        for (auto j = i + 1; j < start_locations.size(); j++)
        {
            ...
            {
                if (getManhattanDistance(paths[a1]->at(t).location, paths[a2]->at(t).location) <= minAgentSeparation) // vertex conflict //Add case for collision at end point 
                {
                ...
                else if (getManhattanDistance(paths[a1]->at(t).location, paths[a2]->at(t - 1).location) <= minAgentSeparation &&
                        getManhattanDistance(paths[a1]->at(t - 1).location, paths[a2]->at(t).location) <= minAgentSeparation) // edge conflict
                {
                    if (num_of_colliding_pairs == 0)
                    {
