...
LNS::LNS(const Instance& instance, double time_limit,
  ...
  if (destory_name == "Adaptive") {                //method used to destroy paths and rebuild them.
    ALNS = true;
  ...
  agents.reserve(N);
  for (int i = 0; i < N; i++) agents.emplace_back(instance, i, use_sipp);
...
bool LNS::run() {
  ...
  if (!succ && initial_solution_runtime < time_limit) {
    if (use_init_lns) {
      ...
      initial_solution_runtime = ((fsec)(Time::now() - start_time)).count();
    } else  // use random restart SS2: Non-default initLNS option
    {
