...
Path SIPP::findPath(const ConstraintTable& constraint_table) {
  ...
  auto start =
      new SIPPNode(start_location, 0, h, nullptr, 0, get<1>(interval),
  ...
  while (!focal_list.empty()) {
    ...
    if (curr->is_goal) {
      ...
      break;
                   ...
                   goal_location &&             // arrive at the goal location
             ...
             curr->location))  // move to neighboring locations
    {
      for (auto& i : reservation_table.get_safe_intervals(
        ...
        auto next_h_val =
            max(my_heuristic[next_location],
                (next_collisions > 0 ? holding_time : curr->getFVal()) -
                    next_timestep);  // path max
        // generate (maybe temporary) node
        auto next = new SIPPNode(next_location, next_timestep, next_h_val, curr,
                                 next_timestep, next_high_generation,
                                 next_high_expansion, next_v_collision,
                                 next_collisions);
        // try to retrieve it from the hash table
    ...
    if (curr->high_expansion == curr->high_generation and
      ...
      auto next_h_val =
          max(my_heuristic[curr->location],
...
Path SIPP::findOptimalPath(const HLNode& node,
                           ...
                           int lowerbound) {
  return findSuboptimalPath(node, initial_constraints, paths, agent, lowerbound,
                            1)
      .first;
}
// find path by SIPP
             ...
             curr->location))  // move to neighboring locations
    {
      for (auto& i : reservation_table.get_safe_intervals(
        ...
        int next_conflicts = curr->num_of_conflicts +
                             ...
                             (int)next_v_collision + (int)next_e_collision;
        auto next =
            new SIPPNode(next_location, next_g_val, next_h_val, curr,
                         next_timestep, next_high_generation,
                         next_high_expansion, next_v_collision, next_conflicts);
        if (dominanceCheck(next))
...
int SIPP::getTravelTime(int start, int end,
  ...
  auto root = new SIPPNode(start, 0, compute_heuristic(start, end), nullptr, 0,
                           1, 1, 0, 0);
    ...
    for (int next_location : next_locations) {
      ...
      if (!constraint_table.constrained(next_location, next_timestep) &&
        ...
        if (next_g_val + next_h_val >=
          ...
          continue;
        auto next = new SIPPNode(next_location, next_g_val, next_h_val, nullptr,
  ...
  for (auto& old_node : ptr->second) {
    if (old_node->timestep <= new_node->timestep and
        old_node->num_of_conflicts <=
            new_node->num_of_conflicts) {  // the new node is dominated by the
                                           // old node
      return false;
    } else if (old_node->timestep >= new_node->timestep and
               old_node->num_of_conflicts >=
                   new_node->num_of_conflicts)  // the old node is dominated by
                                                // the new node
    {                                           // delete the old node
      ...
      num_generated--;  // this is because we later will increase num_generated
                        // when we insert the new node into lists.
      return true;
    } else if (old_node->timestep < new_node->high_expansion and
               new_node->timestep <
                   old_node->high_expansion) {  // intervals overlap --> we need
                                                // to split the node to make
                                                // them disjoint
