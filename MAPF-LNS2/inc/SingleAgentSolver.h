...
class LLNode  // low-level node
{
 public:
  ...
  bool wait_at_goal = false;  // the action is to wait at the goal vertex or
                              // not. This is used for >lenghth constraints
  bool is_goal = false;
  ...
  struct compare_node {
    ...
    bool operator()(const LLNode* n1, const LLNode* n2) const {
      if (n1->g_val + n1->h_val == n2->g_val + n2->h_val) {
        ...
        return n1->h_val >= n2->h_val;  // break ties towards smaller h_vals
                    ...
                    const LLNode* n2) const  // returns true if n1 > n2
    {
      if (n1->num_of_conflicts == n2->num_of_conflicts) {
        ...
        return n1->g_val + n1->h_val >=
               n2->g_val + n2->h_val;  // break ties towards smaller f_vals
...
class SingleAgentSolver {
 public:
  ...
  // virtual Path findOptimalPath(const ConstraintTable& constraint_table, const
  // PathTableWC& path_table) = 0;
  ...
  virtual Path findPath(
      const ConstraintTable& constraint_table) = 0;  // return the path
  void findMinimumSetofColldingTargets(vector<int>& goal_table,
                                       set<int>& A_target);
