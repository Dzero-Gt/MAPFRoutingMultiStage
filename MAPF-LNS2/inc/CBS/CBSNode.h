...
class HLNode  // a virtual base class for high-level node
{
 public:
  ...
  // unordered_map<int, pair<int, int> > conflictGraph; //<edge index, <weight,
  // num of CT nodes> >
...
class CBSNode : public HLNode {
 public:
  ...
  struct compare_node_by_f {
    bool operator()(const CBSNode* n1, const CBSNode* n2) const {
      ...
      /*if (n1->g_val + n1->h_val == n2->g_val + n2->h_val)
      {
              ...
              {
                      if (n1->g_val + n1->cost_to_go == n2->g_val +
      n2->cost_to_go)
                      {
                              return n1->distance_to_go >= n2->distance_to_go;
                      }
                      return n1->g_val + n1->cost_to_go >= n2->g_val +
      n2->cost_to_go;
              }
              return n1->h_val >= n2->h_val;
      }
  ...
  };  // used by CLEANUP to compare nodes by f_val (top of the heap has min
      // f_val)
  ...
  };  // used by FOCAL to compare nodes by distance_to_go (top of the heap has
      // min distance_to_go)
  ...
  };  // used by FOCAL to compare nodes by num_of_collisions (top of the heap
      // has min h-val)

  pairing_heap<CBSNode*, compare<CBSNode::compare_node_by_f> >::handle_type
      cleanup_handle;
  pairing_heap<CBSNode*,
               compare<CBSNode::compare_node_by_inadmissible_f> >::handle_type
      open_handle;
  pairing_heap<CBSNode*, compare<CBSNode::compare_node_by_d> >::handle_type
      focal_handle;
...
struct ConstraintsHasher  // Hash a CT node by constraints on one agent
{
  ...
  struct EqNode {
    bool operator()(const ConstraintsHasher& c1,
                    const ConstraintsHasher& c2) const {
      ...
      while (curr->parent != nullptr) {
        if (get<4>(curr->constraints.front()) == constraint_type::LEQLENGTH ||
            get<4>(curr->constraints.front()) ==
                constraint_type::POSITIVE_VERTEX ||
            get<4>(curr->constraints.front()) ==
                constraint_type::POSITIVE_EDGE ||
