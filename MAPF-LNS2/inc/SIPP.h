 ...
 public:
  ...
  struct eqnode {
    bool operator()(const SIPPNode* n1, const SIPPNode* n2) const {
      ...
      // min(get<1>(n1->interval), get<1>(n2->interval))); //overlapping time
      // intervals
    }
  };
};
...
class SIPP : public SingleAgentSolver {
 ...
 private:
  ...
  typedef boost::heap::pairing_heap<SIPPNode*,
                                    boost::heap::compare<LLNode::compare_node> >
      heap_open_t;
  typedef boost::heap::pairing_heap<
  ...
  typedef boost::unordered_map<SIPPNode*, list<SIPPNode*>, SIPPNode::NodeHasher,
                               SIPPNode::eqnode>
      hashtable_t;
