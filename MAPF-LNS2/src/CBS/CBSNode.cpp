...
void HLNode::updateDistanceToGo() {
  ...
  {
    auto agents = make_pair(min(conflict->a1, conflict->a2),
                            max(conflict->a1, conflict->a2));
  ...
  distance_to_go =
      (int)(conflicts.size() +
            conflicting_agents.size());  // while conflicts only store one
                                         // conflict per pair of agents
}
...
std::ostream& operator<<(std::ostream& os, const HLNode& node) {
  os << "Node " << node.time_generated << " from " << node.chosen_from
     << " ( f = " << node.g_val << " + " << node.h_val
     << ", f hat = " << node.getFHatVal() - node.cost_to_go << " + "
     << node.cost_to_go << ", d = " << node.distance_to_go << " ) with "
